...
const click_counter = ref(0);

watch(categoryMenuStore, async () => {
  for (const category in categoryMenuStore.categoryMenuState){
    if (categoryMenuStore.categoryMenuState[category] === true){
      try{
        items.value = await axios.get(`http://127.0.0.1:8000/items/?category_filter=${category}`).then(res => res.data)
      }
      catch (error) {
        console.log(error);
      }
...
watch(click_counter, async () => {
  if (cart_id.value === 0){
    cart_id.value = await axios.post("http://127.0.0.1:8000/cart/").then((res) => res.data);
  }
  else {
    await axios.put(`http://127.0.0.1:8000/cart/${cart_id.value}/?operation=add&item_id=${cart_item_id.value}`).then((res) => res.data);
    cart.value = await axios.get(`http://127.0.0.1:8000/cart/${cart_id.value}/`).then((res) => res.data);
  }
...
<template>
  ...
  <div class="item-wrapper">
    <li v-for="item in items">
      <Item class="item" :item_id="item.id" v-model:cart_item_id="cart_item_id" v-model:click_counter="click_counter">
  ...
  <div class="cart-wrapper">
    ...
    <li v-for="cart_item in cart">
      <CartItem :unitPrice="cart_item.price" :quantity="cart_item.quantity">
